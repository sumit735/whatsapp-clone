...
import MessageSingle from '../Message/MessageSingle';
    ...
    const showMessages = () => {
        if(messageSnapshot) {
            return messageSnapshot.docs.map(msg => (
                <MessageSingle key={msg.id} user={msg.data().user} msg={{...msg.data(), timestamp: msg.data().timestamp?.toDate().getTime()}} />
            ))
        } else {
            return JSON.parse(messages).map( msg => (
                <MessageSingle key={msg.id} user={msg.user} msg={msg} />
            ))
        }
    }
